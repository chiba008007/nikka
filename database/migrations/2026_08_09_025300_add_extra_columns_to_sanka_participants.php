...
return new class () extends Migration {
    ...
    public function up(): void
    {
        ...
        $extraColumns = [];
        for ($i = 1; $i <= 50; $i++) {
            $extraColumns[] = "add_column_{$i}";
        }

        // 本体テーブルに追加
        Schema::table('sanka_participants', function (Blueprint $table) use ($extraColumns) {
            // 予備カラム50個
            ...
            // 参加可否
            $table->unsignedTinyInteger('participation_status')->nullable();

            // 懇親会参加可否
            $table->unsignedTinyInteger('banquet_status')->nullable();
        });

        // 履歴テーブルにも追加
        Schema::table('sanka_participant_histories', function (Blueprint $table) use ($extraColumns) {
            // 予備カラム50個
            foreach ($extraColumns as $column) {
                $table->text($column)->nullable();
            }
        ...
        // トリガーを作り直す
        $this->dropTriggers();
        $this->createTriggers($triggerColumns);
    }
    ...
    public function down(): void
    {
        ...
        // 履歴テーブルから削除
        Schema::table('sanka_participant_histories', function (Blueprint $table) use ($dropColumns) {
            $table->dropColumn($dropColumns);
        });

        // 本体テーブルから削除
        Schema::table('sanka_participants', function (Blueprint $table) use ($dropColumns) {
            $table->dropColumn($dropColumns);
        });
     ...
     * 履歴保存トリガーを作成する
     */
    private function createTriggers(array $extraColumns): void
    {
        // 履歴テーブルへ保存する予備カラム
        $extraInsertColumns = '';
        $extraNewValues = '';
